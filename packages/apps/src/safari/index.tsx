 ...
 * Opens to Google homepage with working search.
 */

import React, { useState, useRef, useEffect } from 'react';
import { ZWindow } from '@z-os/ui';
import { ChevronLeft, ChevronRight, RotateCw, Share, Plus, Lock, Search, ExternalLink, Compass } from 'lucide-react';
...
const getSearchQuery = (url: string): string | null => {
  try {
    const urlObj = new URL(url);
    // Check for internal search URL format
...
const SafariWindow: React.FC<SafariWindowProps> = ({ onClose, onFocus }) => {
  ...
  const handleSearch = (query: string) => {
    // Use internal search URL to trigger SearchResults component
  ...
  const handleUrlSubmit = (e: React.FormEvent) => {
    ...
    // If it looks like a search query (no dots, has spaces), search for it
